...
namespace Inventory {
    ...
    public class InventoryController : Singleton<InventoryController> {
        ...
        [Header("Hotbar Inventory")] public GridInv primary;
        ...
        [Header("Inventory Reference for enabling/disabiling the entire UI and item spawning")] [SerializeField]
        private GameObject inventory;

        [SerializeField] private GameObject chestUI;


        [Header("Item datas")] public List<ItemData> items;
        ...
        [Header("Keybinds")] [SerializeField] private KeyCode generateItem = KeyCode.Q;

        [SerializeField] private KeyCode rotate = KeyCode.R;
        [SerializeField] private KeyCode OpenCloseInventory = KeyCode.E;

        private Item currentitem;
        private GameObject instaniatedToolTip;
        ...
        private void Update() {
            ...
            if (inventory.activeInHierarchy) //Only run when the inventory is open
            {
                ...
                if (selectedItemGrid == null)
                    return; //Prevents the following code being run if there is no grid selected, reduces the need for validation

                if (Input.GetMouseButtonDown(0)) ClickItem();
                else if (Input.GetMouseButtonDown(1))
                    RightClickItem(selectedItemGrid.GetTileGridPosition(Input.mousePosition), selectedItemGrid);
                else ToolTip();
            }
            else {
                DestroyToolTip();
        ...
        private void SetPlayerWeapon() {
            var hand = player.transform.GetChild(0);
            var weaponSprite = hand.GetComponent<SpriteRenderer>();

            try {
                if (!player.GetComponent<Controller>().removeItem) {
                    if (player.GetComponent<Controller>().hotbar == 1)
                        weaponItem = primary.GetItem(0, 0); //Hotbar Keybinds
                    ...
                    hand.transform.localScale = new Vector3(0.1f * weaponItem.itemdata.width,
                        0.1f * weaponItem.itemdata.width, 0.1f);
                    ...
                    else if (player.GetComponent<Controller>().hotbar == 2)
                        RightClickItem(new Vector2Int(0, 0), hotbar);
                    else if (player.GetComponent<Controller>().hotbar == 3)
                        RightClickItem(new Vector2Int(1, 0), hotbar);
                    else if (player.GetComponent<Controller>().hotbar == 4)
                        RightClickItem(new Vector2Int(2, 0), hotbar);
                    else if (player.GetComponent<Controller>().hotbar == 5)
                        RightClickItem(new Vector2Int(3, 0), hotbar);
                    else if (player.GetComponent<Controller>().hotbar == 6)
                        RightClickItem(new Vector2Int(4, 0), hotbar);
        ...
        private void ToolTip() {
            ...
            try {
                ...
                if (tooltip != null) {
                    ...
                    Destroy(instaniatedToolTip); //Delete when item is changed or nulled
        ...
        private void ItemIconDrag() {
            if (selectedItem != null) rectTransform.position = Input.mousePosition;
        }
        ...
        private void Rotate() {
            if (selectedItem == null) return;
            if (selectedItem.itemdata.width == selectedItem.itemdata.height)
                return; //Dont rotate if item is a square
            rectTransform = selectedItem.GetComponent<RectTransform>();

            if (selectedItem.GetComponent<Item>().rotated) {
                selectedItem.GetComponent<Item>().rotated = false;
                rectTransform.pivot = new Vector2(0f, 1f);
                rectTransform.transform.eulerAngles = new Vector3(selectedItem.transform.eulerAngles.x,
                    selectedItem.transform.eulerAngles.x, 0f);
            }
            else {
                selectedItem.GetComponent<Item>().rotated = true;
                rectTransform.pivot = new Vector2(1f, 1f);
                rectTransform.transform.eulerAngles = new Vector3(selectedItem.transform.eulerAngles.x,
                    selectedItem.transform.eulerAngles.x, 90f);
            }
        }
    }
}